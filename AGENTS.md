...
- ⏳ **Отложено, волна 2**: страницы модулей Питание, Покупки, Баллы (лента), Награды, Достижения (полная), Хранилище, Ассистент/чат; онбординг; письма/уведомления Laravel (`app/Mail`, `app/Notifications` — шаблоны и темы); демо-витрина `/design-system/*`; мелкие остатки.
