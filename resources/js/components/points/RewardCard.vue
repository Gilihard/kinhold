  ...
  >
    <div class="flex items-start justify-between mb-2">
      ...
      <div v-if="isParent" class="flex items-center gap-1.5">
        <button class="p-1 rounded hover:bg-lavender-100 dark:hover:bg-prussian-700 text-lavender-400 hover:text-wisteria-500 dark:hover:text-wisteria-400" aria-label="Изменить награду" @click="$emit('edit', reward)">
          <PencilIcon class="w-4 h-4" />
        </button>
        <button class="p-1 rounded hover:bg-red-50 dark:hover:bg-red-900/20 text-lavender-400 hover:text-red-500" aria-label="Удалить награду" @click="$emit('delete', reward.id)">
    ...
    <div class="flex items-center justify-between mt-auto">
      <span class="text-sm font-bold font-mono text-wisteria-600 dark:text-wisteria-400">{{ formatPts(reward.point_cost) }}</span>
      <button
  ...
  >
    ...
    >
      <div class="flex items-center gap-2">
        ...
        >
          {{ reward.bid_end_at ? 'С таймером' : 'Без таймера' }}
        </span>
      ...
      <div v-if="isParent" class="flex items-center gap-1">
        <button class="p-1 rounded hover:bg-wisteria-200/50 dark:hover:bg-wisteria-800/30 text-lavender-400 hover:text-wisteria-500 dark:hover:text-wisteria-400" aria-label="Изменить аукцион" @click="$emit('edit', reward)">
          <PencilIcon class="w-4 h-4" />
        </button>
        <button class="p-1 rounded hover:bg-red-100/50 dark:hover:bg-red-900/20 text-lavender-400 hover:text-red-500" aria-label="Удалить аукцион" @click="$emit('delete', reward.id)">
          <TrashIcon class="w-4 h-4" />
        </button>
      </div>
    </div>
    ...
    <div class="p-4">
      <div class="flex flex-col sm:flex-row sm:gap-6">
        ...
        <div v-if="!reward.is_resolved" class="sm:w-48 shrink-0 space-y-1.5 bg-lavender-50 dark:bg-prussian-800/50 rounded-lg p-3">
          <div v-if="reward.highest_bid" class="flex justify-between text-xs">
            <span class="text-lavender-500 dark:text-lavender-400">Высшая ставка</span>
            <span class="font-bold font-mono text-wisteria-600 dark:text-wisteria-400">{{ formatPts(reward.highest_bid) }}</span>
          </div>
          <div v-else class="text-xs text-lavender-400 dark:text-lavender-500 text-center py-1">Ставок пока нет</div>

          <!-- Leading bidder (parent view) -->
          <div v-if="isParent && reward.highest_bidder" class="flex justify-between text-xs">
            <span class="text-lavender-500 dark:text-lavender-400">Лидер</span>
          ...
          <div v-if="reward.total_bids" class="flex justify-between text-xs">
            <span class="text-lavender-500 dark:text-lavender-400">Всего ставок</span>
          ...
          <div v-if="reward.min_bid && !reward.highest_bid" class="flex justify-between text-xs">
            <span class="text-lavender-500 dark:text-lavender-400">Мин. ставка</span>
            <span class="font-medium text-prussian-500 dark:text-lavender-300">{{ formatPts(reward.min_bid) }}</span>
          </div>
          ...
          <div v-if="reward.my_bid" class="flex justify-between text-xs pt-1.5 border-t border-lavender-200 dark:border-prussian-600">
            <span :class="reward.is_leading ? 'text-green-600 dark:text-green-400' : 'text-golden-600 dark:text-golden-400'">
              {{ reward.is_leading ? 'Вы лидируете!' : 'Ваша ставка' }}
            </span>
        ...
        <div v-else class="sm:w-48 shrink-0 flex items-center justify-center bg-lavender-50 dark:bg-prussian-800/50 rounded-lg p-3">
          <span class="text-sm font-medium text-lavender-500 dark:text-lavender-400">Аукцион завершён</span>
        </div>
      </div>
      ...
      <div v-if="reward.bidding_open" class="flex items-center justify-between mt-4 pt-3 border-t border-lavender-100 dark:border-prussian-700">
        ...
        <div>
          ...
          >
            {{ reward.my_bid ? 'Поднять ставку' : 'Сделать ставку' }}
          </button>
...
import { formatPts, pluralRu } from '@/utils/plural'
...
const stockLabel = computed(() => {
  ...
  if (remaining === 0) return 'Распродано'
...
const expiryLabel = computed(() => {
  ...
  if (props.reward.is_expired) return 'Срок истёк'
  ...
  if (daysLeft <= 7) return `Осталось ${daysLeft} ${pluralRu(daysLeft, 'день', 'дня', 'дней')}`
  return `Истекает ${expDate.toLocaleDateString('ru-RU', { day: 'numeric', month: 'short' })}`
})
...
const countdownLabel = computed(() => {
  ...
  if (!props.reward.bid_end_at) return props.reward.bidding_open ? 'Закрывает родитель' : null
  ...
  if (hours < 1) {
    ...
    return `Осталось ${minsLeft} ${pluralRu(minsLeft, 'минута', 'минуты', 'минут')}`
  }
  if (hours < 24) return `Осталось ${hours} ${pluralRu(hours, 'час', 'часа', 'часов')}`
  if (days === 1) return 'Завершится завтра'
  if (days <= 7) return `Осталось ${days} ${pluralRu(days, 'день', 'дня', 'дней')}`
  return `Завершится ${target.toLocaleDateString('ru-RU', { day: 'numeric', month: 'short' })}`
})
...
const visibilityLabel = computed(() => {
  ...
  if (v === 'parent_only') return 'Только родители'
  if (v === 'child_only') return 'Только дети'
  if (v === 'specific' && props.reward.visible_to_names?.length) {
    return `Для ${props.reward.visible_to_names.join(', ')}`
  }
  if (v === 'specific') return 'Конкретные участники'
  return null
})
</script>
