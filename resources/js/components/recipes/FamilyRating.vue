<template>
  <div class="space-y-3">
    ...
    <div class="flex items-center gap-4">
      <!-- Stars -->
      <div class="flex items-center gap-0.5" role="group" aria-label="Оценить этот рецепт">
        <button
          v-for="star in 5"
          :key="star"
          class="p-0.5 transition-colors focus:outline-none focus:ring-2 focus:ring-[#C4975A]/30 rounded"
          :aria-label="`Оценить на ${star} ${pluralRu(star, 'звезду', 'звезды', 'звёзд')}`"
    ...
    <div v-if="ratings.length > 0">
      ...
      >
        {{ showAll ? 'Скрыть' : 'Показать' }} оценки семьи ({{ ratings.length }})
      </button>
